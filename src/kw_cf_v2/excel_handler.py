...
def read_keywords(file_info:FileInfo) -> SourceKeywordDTO:
    ...
    for item in records:
        ...
        temp_item.keyword = item.pop('关键词')
...
def read_work_flow_rules(file_info:FileInfo):
    ...
    for sheet_name in sheet_names:
        ...
        for record in records:
            ...
            temp_item.rule = record.pop('分类规则')
            temp_item.target_file_name = record.pop('结果文件名称')
            temp_item.target_sheet_name = record.pop('结果sheet名称') if record.get('结果sheet名称') else 'Sheet1'
            temp_item.limit_last_matched_rule = record.pop('上层分类规则') if record.get('上层分类规则') else ''
            temp_item.rule_tag = record.pop('分类标签') if record.get('分类标签') else ''
            result.append(temp_item)
...
def save_classified_keywords(classified_keywords:ClassifiedKeywordDTO,time_str:str,is_create_new_file:bool=False):
    ...
    for output_file_name,sheet_dict in result.items(): 
        for sheet_name,data in sheet_dict.items():
            ...
            df.to_excel(file_path_map[output_file_name],sheet_name=sheet_name,index=False)
